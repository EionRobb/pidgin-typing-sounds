...
PIDGIN_CFLAGS = -I/usr/include/pidgin -I/usr/local/include/pidgin -I/usr/include/gtk-2.0/ -I/usr/include/cairo -I/usr/include/pango-1.0 -I/usr/include/atk-1.0
#LIBPURPLE_CFLAGS = `pkg-config --cflags pidgin`
...
all:	typewriter-sounds.dll typewriter-sounds.so

install:
	cp typewriter-sounds.so /usr/lib/pidgin/
clean:
	rm -f typewriter-sounds.dll typewriter-sounds.so

typewriter-sounds.so:	${SOURCES}
	${LINUX32_COMPILER} ${LIBPURPLE_CFLAGS} ${PIDGIN_CFLAGS} -Wall ${GLIB_CFLAGS} -I. -g -O2 -pipe ${SOURCES} -o $@ -shared -fPIC -DPIC

typewriter-sounds.dll:	${SOURCES}
	${WIN32_COMPILER} ${LIBPURPLE_CFLAGS} ${PIDGIN_CFLAGS} -Wall -I. -g -O0 -pipe ${SOURCES} -o $@ -shared -mno-cygwin ${WIN32_CFLAGS} ${WIN32_LIBS}
	upx $@
